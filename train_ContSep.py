...
def parse_args():
    ...
    parser.add_argument("--ctx_weight", type=float, default=1)
    parser.add_argument("--ce", default=True)
...
def train_net(args):
    ...
    if args.train_data == 'dailytalk':
        args.ce = False 

    model = ContSep(add_mt=True, num_spks=args.num_max_mix, ce=args.ce)
    if args.from_ckpt:
    ...
    if scaler_state_dict is not None:
        if args.rank == 0:
            print('* Scaler is loaded from ckpt')
    ...
    _ = validate(model, llm, fast_validate=True, val_dataset=args.train_data)    # debug the pipeline
    train(model, llm, train_data, args.epochs, optimizer=optimizer, scheduler=scheduler, args=args, scaler=scaler)

def train(model, llm, train_data, epochs, optimizer, scheduler, args, scaler):
    best_val_sisnr = 0.
    now = datetime.now()
    ...
    sp_criterion = speechbrain.nnet.losses.get_si_snr_with_pitwrapper
    if not args.ce:
        selection_criterion = nn.BCEWithLogitsLoss().cuda()
    else:
        selection_criterion = nn.CrossEntropyLoss().cuda()
    ...
    for epoch in range(args.start_epoch, epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if args.num_max_mix > 2:
                mixed_sp, gt_sp, context, context_mask, sp_len, _, ns_sp_1, ns_sp_2 = batch
            else:
                mixed_sp, gt_sp, context, context_mask, sp_len, _, ns_sp_1 = batch
            ...
            with sdpa_kernel(SDPBackend.FLASH_ATTENTION) if args.fp16 or args.bf16 else contextlib.nullcontext(), torch.autocast(device_type='cuda', dtype=torch.float16 if args.fp16 else torch.bfloat16) if args.fp16 or args.bf16 else contextlib.nullcontext():
                ...
                sisnrs = -1. * sp_si_snr_criterion(gt_sp.unsqueeze(-1).repeat(1, 1, args.num_max_mix).transpose(0, 1).cuda(), enhanced_sp.transpose(0, 1).float()).squeeze(0).detach()
                context_index = sisnrs.argmax(-1)
                ctx_loss = selection_criterion(ctx_pred.squeeze(1), context_index.float() if args.num_max_mix == 2 and not args.ce else context_index)
                
                if args.num_max_mix > 2:
                    snr_loss = sp_criterion(enhanced_sp, torch.stack([gt_sp, ns_sp_1, ns_sp_2], dim=-1).cuda()).mean()
                else:
                    snr_loss = sp_criterion(enhanced_sp, torch.stack([gt_sp, ns_sp_1], dim=-1).cuda()).mean()
                loss = args.ctx_weight * ctx_loss + snr_loss
            ...
            snr_loss = snr_loss.cpu().item()
            snr_loss_list.append(snr_loss)
            ctx_loss = ctx_loss.cpu().item()
            ctx_loss_list.append(ctx_loss)
            
            if not args.ce:
                ctx_pred = (nn.functional.sigmoid(ctx_pred.cpu().squeeze(-1)) > 0.5).int()
            else:
                ctx_pred = (nn.functional.softmax(ctx_pred.cpu().squeeze(-1), dim=-1).argmax(-1))

            ctx_acc = (ctx_pred == context_index.cpu()).float().mean()

            if args.rank == 0 and writer is not None:
                writer.add_scalar('train/loss', loss, step)
                writer.add_scalar('train/snr_loss', snr_loss, step)
                writer.add_scalar('train/ctx_loss', ctx_loss, step)
                writer.add_scalar('train/SI-SNR', -1. * snr_loss, step)
                writer.add_scalar('train/ctx_acc', ctx_acc, step)
                ...
                if step % 100 == 0:
                    ...
                    if wandbrun is not None:
                        ...
                        wandbrun.log({'train/snr_loss': snr_loss}, step)
                        wandbrun.log({'train/ctx_loss': ctx_loss}, step)
                        wandbrun.log({'train/SI-SNR': -1. * snr_loss}, step)
                        wandbrun.log({'train/ctx_acc': ctx_acc}, step)
                        wandbrun.log({'train/learning_rate': optimizer.param_groups[0]['lr']}, step)
            
            if step % args.eval_step == 0:
                val_sisnr = validate(model, llm, epoch=epoch, writer=writer, wandbrun=wandbrun, step=step, val_dataset=args.train_data, fast_validate=True)
                ...
                if args.rank == 0:
                    ...
                    torch.save(
                        ...
                        'scaler': scaler_state_dict if scaler is not None else None,
                        'step': step,
                        'epoch': epoch,
                        },
            ...
            if args.generate_speech and step % args.generate_step == 0:
                if args.rank == 0:
                    ...
                    for kk, (gt_sp, pred_sp, mixed, length) in enumerate(zip(gt_sp, enhanced_sp.cpu().detach(), mixed_sp, sp_len)):
                        ...
                        pred_save_names = []
                        for pp in range(pred_sp.size(-1)):
                            pred_save_names.append(os.path.join(args.temp_dir, 'train',f'preds_{pp + 1}', f'{kk}.wav'))
                        ...
                        for pred_save_name in pred_save_names:
                        ...
                        for pp in range(pred_sp.size(-1)):
                            pred_s = pred_sp[:, pp].float().numpy()
                            pred_s = pred_s / np.max(np.abs(pred_s)) * 0.9
                            sf.write(pred_save_names[pp], pred_s, samplerate=args.sr, subtype='PCM_16')
                        
                            if wandbrun is not None:
                                if kk < 3:
                                    wandbrun.log({f'train/Pred_{pp + 1}/{kk}': wandb.Audio(pred_s, sample_rate=args.sr)}, step)
...
def validate(model, llm, fast_validate=False, epoch=0, writer=None, wandbrun=None, step=0, val_dataset='spokenwoz'):
    ...
    with torch.no_grad():
        ...
        batch_size = dataloader.batch_size
        if fast_validate:
            samples = min(100 * batch_size, int(len(dataloader.dataset)))
            max_batches = 100
        else:
        ...
        for i, batch in enumerate(dataloader):
            ...
            if args.num_max_mix > 2:
                mixed_sp, gt_sp, context, context_mask, sp_len, f_name, ns_sp_1, ns_sp_2 = batch
            else:
                mixed_sp, gt_sp, context, context_mask, sp_len, f_name, ns_sp_1 = batch
            ...
            sisnrs = -1. * sp_si_snr_criterion(gt_sp.unsqueeze(-1).repeat(1, 1, args.num_test_mix).transpose(0, 1).cuda(), enhanced_sp.transpose(0, 1).float()).squeeze(0)
            context_index = sisnrs.argmax(-1)
            ...
            val_ctx_acc_list.append(ctx_acc)
            ...
            if args.generate_speech and len(mixed) < args.num_gen_speech:
                ctx_preds.extend([sp for sp in ctx_pred])
        ...
        if args.generate_speech:
            if args.rank == 0:
                ...
                for kk, (gt_sp, pred_sp, mixed_sp, sp_len, f_name, ctx_pred) in enumerate(zip(gts, preds, mixed, sp_lens, f_names, ctx_preds)):
                    ...
                    if wandbrun is not None:
                        if kk < 3:
                            ...
                            wandbrun.log({f'val_{val_dataset}/Pred/{kk}': wandb.Audio(pred_sp, sample_rate=args.sr, caption=f"Stream {ctx_pred.item()} is choosed")}, step)
                            wandbrun.log({f'val_{val_dataset}/Mixed/{kk}': wandb.Audio(mixed_sp, sample_rate=args.sr)}, step)

        if args.distributed:
            dist.barrier()

        val_ctx_acc = np.mean(val_ctx_acc_list)
        val_sisnr = np.mean(val_sisnr_list)
        prev_sisnr = np.mean(prev_sisnr_list)
        if args.rank == 0:
            print(f"## VALIDATION SI-SNR ({val_dataset}): ", val_sisnr)      
            print(f"## VALIDATION CTX ACC ({val_dataset}): ", val_ctx_acc)

        if args.rank == 0 and writer is not None:
            writer.add_scalar(f'val_{val_dataset}/CTX_ACC', val_ctx_acc, step)
            ...
            if wandbrun is not None:
                wandbrun.log({f'val_{val_dataset}/CTX_ACC': val_ctx_acc}, step)
